...
const matchTrackingData = (trackingNumber, regex) => {
    const r = (0, ramda_1.is)(String, regex)
        ? regex
        : regex.join('');
...
const formatList = (tracking) => (0, ramda_1.pipe)((0, ramda_1.split)(''), (0, ramda_1.map)((x) => isNaN(x)
    ? (x.charCodeAt(0) - 3) % 10
    : parseInt(x)))(tracking);
const toObj = (list) => Object.assign({}, list);
const evenKeys = (_v, k) => k % 2 === 0;
const oddKeys = (0, ramda_1.complement)(evenKeys);
const getSum = (parityFn, tracking) => (0, ramda_1.pipe)(toObj, 
// @ts-ignore Bad Ramda types
(0, ramda_1.pickBy)(parityFn), ramda_1.values, ramda_1.sum)(tracking);
const mod10 = ({ serial, checkDigit, checksum }) => {
    const t = formatList(serial.replace(/[^\da-zA-Z]/g, ''));
    const keySum = (0, ramda_1.sum)([
...
const addWeight = (weightings, serial) => (0, ramda_1.sum)((0, ramda_1.zip)(serial.split('').map(s => parseInt(s)), weightings || []).map(x => x.reduce(ramda_1.multiply)));
...
const getTrackingList = (searchText) => (trackingData) => (0, ramda_1.pipe)((0, ramda_1.prop)('regex'), (0, ramda_1.ifElse)((0, ramda_1.is)(String), ramda_1.identity, (0, ramda_1.join)('')), (r) => new RegExp(r, 'g'), (0, ramda_1.flip)(ramda_1.match)(searchText), (0, ramda_1.map)((0, ramda_1.replace)(/[^a-zA-Z\d\n\r]/g, '')), ramda_1.uniq)(trackingData);
const getCourierList = (searchText, couriers) => couriers.map((0, ramda_1.pipe)((0, ramda_1.prop)('tracking_numbers'), (0, ramda_1.chain)((0, ramda_1.pipe)(getTrackingList(searchText), ramda_1.flatten))));
const findTrackingMatches = (searchText, couriers) => (0, ramda_1.pipe)(ramda_1.flatten, ramda_1.uniq, (a) => (0, ramda_1.filter)((t) => (0, ramda_1.none)((0, ramda_1.test)(new RegExp(`([a-zA-Z0-9 ]+)${t}$`)), a)
// @ts-ignore Bad Dictionary Type
)(a), (a) => (0, ramda_1.filter)((t) => (0, ramda_1.none)((0, ramda_1.test)(new RegExp(`^${t}([a-zA-Z0-9 ]+)`)), a)
// @ts-ignore Bad Dictionary Type
)(a))(getCourierList(searchText, couriers));
// eslint-disable-next-line @typescript-eslint/explicit-function-return-type
const getTrackingInternal = (trackingNumber) => (0, ramda_1.reduce)((prev, courier) => (prev || (0, ramda_1.reduce)((_, tn) => {
    const serialData = getSerialData(trackingNumber, tn);
    return (serialData && validator(tn)(serialData) && additional(trackingNumber, tn))
        ? (0, ramda_1.reduced)(toTrackingNumber(tn, courier, trackingNumber))
        : undefined;
}, undefined, courier.tracking_numbers)), undefined);
const getTracking = (trackingNumber, couriers = exports.allCouriers) => (getTrackingInternal(trackingNumber)(couriers));
exports.getTracking = getTracking;
const findTracking = (searchText, couriers) => findTrackingMatches(searchText, couriers || exports.allCouriers)
    .map(t => (0, exports.getTracking)(t, couriers || exports.allCouriers))
    .filter((0, ramda_1.complement)(ramda_1.isNil));
